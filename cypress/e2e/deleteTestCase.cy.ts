describe("Eliminar Caso de Prueba Funcional", () => {
    ...
    it("Eliminar Caso de Prueba Funcional", () => {
      ...
      cy.get("form").within(() => {
        cy.get('input[name="email"]').should("be.visible").type("juan.f.castillo.e@unl.edu.ec");
        cy.get('input[name="password"]').should("be.visible").type("123456");
        cy.get('button[type="submit"]').contains("Iniciar sesión").should("be.visible").click();
      });
      ...
      cy.wait(2000); 
      cy.contains("Eliminar", { timeout: 10000 }).eq(0).should("be.visible").click(); // 
      cy.contains("Eliminar").eq(1).should("be.visible").click();
      cy.contains("Eliminar").eq(2).should("be.visible").click(); 
    });
  });
